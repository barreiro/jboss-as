...
/**
 * A common test bean used by the enc servlets to validate their enc
 * configuration.
 * 
 * @author Scott.Stark@jboss.org
 */
public class ENCTesterFirst {
    
    private Logger log;

    public ENCTesterFirst(Logger log) {
        this.log = log;
    }

    public void testENC() throws FacesException {
        try {
            // Obtain the environment naming context.
            Context initCtx = new InitialContext();
            log.info("InitialContext.env:");
            for (Object key : initCtx.getEnvironment().keySet()) {
                log.info("Key: " + key + ", value: " + initCtx.getEnvironment().get(key));
            }
