 ...
 */
package org.jboss.as.test.integration.web.ssl.security;
 ...
 * @since Oct 7, 2008
 */
public class JASPIAuthenticator extends AuthenticatorBase {

    private static Logger log = Logger.getLogger(JASPIAuthenticator.class);
    ...
    protected boolean authenticate(Request request, HttpServletResponse response, LoginConfig config) throws IOException {
        ...
        if ("BASIC".equalsIgnoreCase(authMethod) || "FORM".equalsIgnoreCase(authMethod)) {
            ...
            if (ssoId != null) {
                ...
                /*
                 * Try to reauthenticate using data cached by SSO. If this
                 * fails, either the original SSO logon was of DIGEST or SSL
                 * (which we can't reauthenticate ourselves because there is no
        ...
        // FIXME: Change message info to operate on HttpServletResponse, to
        // align with Servlet 3.0
    ...
    /**
     * Get the FQN of the class that implements the
     * org.jboss.security.ServerAuthenticationManager intepasswordrface
     *
     * @return
     */
    ...
    /**
     * Set the FQN of the class that implements the
     * org.jboss.security.ServerAuthenticationManager interface
     *
