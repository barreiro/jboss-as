 ...
 * <p/>
 * If the table used for persistence here does not exist, ejbFindAll() will
 * create it.
 * 
 * @author $Author: dimitris@jboss.org $
 */

public class CtsBmpBean implements EntityBean {

    private static final long serialVersionUID = 2297118413783599706L;
     ...
     * @throws DuplicateKeyException If another instance with this primary key
     *         already exist.
     */
    public AccountPK ejbCreate(AccountPK pk, String personsName) throws CreateException, DuplicateKeyException {
        log.debug("entry ejbCreate(\"" + pk.getKey() + "\", " + "\"" + personsName + "\")");
        ...
        try {
            ...
            try {
                ...
                ps = con.prepareStatement("SELECT accountNumber " + "FROM " + TABLE_NAME + " " + "WHERE accountNumber=?");
                try {
     ...
     * @throws ObjectNotFoundException If no instance with this primary key
     *         exists.
     ...
     * @throws FinderException If the lookup failed.
     */
    public Collection<AccountPK> ejbFindAll() throws FinderException {
        log.debug("entry ejbFindAll");

        ensureTableExists();

        Collection<AccountPK> result = new LinkedList<AccountPK>();
        try {
            Connection con = ds.getConnection();
            try {
                PreparedStatement ps;

                ps = con.prepareStatement("SELECT accountNumber " + "FROM " + TABLE_NAME);
                try {
                    ResultSet rs = ps.executeQuery();
    ...
    /**
     * Find all instances where the personsName property is equal to the
     * argument.
     * 
     * @throws FinderException If the lookup failed.
     */
    public Collection<AccountPK> ejbFindByPersonsName(String guysName) throws FinderException {
        log.debug("entry ejbFindByPersonsName(\"" + guysName + "\").");

        Collection<AccountPK> result = new java.util.LinkedList<AccountPK>();
        try {
            Connection con = ds.getConnection();
            try {
                PreparedStatement ps;

                ps = con.prepareStatement("SELECT accountNumber " + "FROM " + TABLE_NAME + " " + "WHERE name=?");
                try {
     ...
     */
    public void ejbLoad() {
        log.debug("ejbLoad(\"" + ((AccountPK) ctx.getPrimaryKey()).getKey() + "\") called");
        ...
        try {
            ...
            try {
                ...
                ps = con.prepareStatement("SELECT accountNumber,name " + "FROM " + TABLE_NAME + " " + "WHERE accountNumber=?");
                try {
        ...
        log.debug("ejbLoad(\"" + ((AccountPK) ctx.getPrimaryKey()).getKey() + "\") returning");
    ...
    public void ejbStore() {
        ...
        try {
            ...
            try {
                ...
                ps = con.prepareStatement("UPDATE " + TABLE_NAME + " " + "SET name=? " + "WHERE accountNumber=?");
                try {
    ...
    public void ejbRemove() {
        ...
        try {
            ...
            try {
                ...
                ps = con.prepareStatement("DELETE FROM " + TABLE_NAME + " " + "WHERE accountNumber=?");
                try {
    ...
    private void initializeDatabaseTable() {
        ...
        try {
            ...
            try {
                ...
                s = con.createStatement();
                try {
                    s.executeUpdate("CREATE TABLE " + TABLE_NAME + " " + "(accountNumber VARCHAR(25)," + " name VARCHAR(200))");
